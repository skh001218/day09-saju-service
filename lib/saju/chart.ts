...
export function validateInput(raw: SajuInput): SajuInput {
  ...
  if (typeof date !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(date))
    throw new InputError("생년월일은 YYYY-MM-DD 형식으로 입력해 주세요.", "date");
  ...
  if (date > todayInKorea())
    throw new InputError("미래 날짜는 생년월일로 입력할 수 없습니다.", "date");
  ...
  if (
    ...
    throw new InputError("태어난 시각은 24시간제 HH:mm 형식으로 입력해 주세요.", "time");
